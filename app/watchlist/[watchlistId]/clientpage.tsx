...
export default function ClientPage(props: { watchlist: Watchlist }) {
    ...
    async function onNameChanged(e: any) {
        ...
        setTimeout(async () => {
            ...
            const promise = fetch(`/api/watchlist/updatename?name=${name}&id=${watchlist._id}`);
    ...
    async function updateWatchlist(e: any, symbols: string[] = getSymbols()) {
        ...
        if(e.target.value === "") {
            ...
            const promise = fetch(`/api/watchlist/updatesymbols?id=${watchlist._id}&symbols=${JSON.stringify(symbols)}`);
            toast.promise(promise, {
        ...
        return <div id="symbols">
            {
                symbols.map((symbol, index) => 
                    <div id={symbol} key={index} className='flex flex-row items-center justify-center mb-1'>
                        ...
                        <button className="btn btn-error" onClick={(e) => deleteSymbol(e, symbol)}>
                            Remove
                        </button>
                    </div>)
            }
    ...
    useEffect(() => {
        router.events.on("routeChangeStart", () => updateWatchlist(null, getSymbols()));
    });

    return <div id="main" className='flex-1 flex items-center justify-center flex-col space-y-2'>
            <input type="text" placeholder="Unnamed Watchlist" defaultValue={watchlist.name} onChange={onNameChanged} 
                className="input input-bordered w-full text-primary" />
            <button className="btn btn-error" onClick={deleteWatchlist}>Delete Watchlist</button>
            <span className="text-primary">Symbols:</span>
            {symbolListElement()}
            <button className="btn btn-primary" onClick={addSymbol}>Add Symbol</button>
        </div>
}